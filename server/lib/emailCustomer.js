...
export const sendCustomerEmail = async (order) => {
  ...
  const subject = isCOD
    ? `✅ QuanNguyenS — Xác nhận đơn hàng #${order.orderId}`
    : `✅ QuanNguyenS — Đặt hàng thành công #${order.orderId}`

  const html = isCOD ? buildCODEmailHTML(order) : buildPaidEmailHTML(order)
