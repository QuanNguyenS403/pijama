...
export default function AccountPromptBanner() {
  ...
  // 'GOOGLE_DIRECT' = xác nhận họ tên Google & email
  // 'FACEBOOK_DIRECT' = xác nhận họ tên Facebook & email
  // 'VERIFY_CODE' = form nhập mã xác minh 6 số gửi về email (backward compatibility)
  // 'SUCCESS' = màn hình chúc mừng nhận voucher & hiển thị tên tài khoản đã tạo
  ...
  const handleGoogleCredentialResponse = async (response) => {
    ...
    try {
      ...
      // Nếu server đã tạo/đăng nhập tài khoản trực tiếp
      ...
      // Backward compatibility nếu server yêu cầu OTP
      ...
      try {
        ...
        window.google.accounts.id.prompt((notification) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // Popup bị chặn hoặc One Tap skip -> chuyển sang form xác nhận Google
            setProvider('google')
        ...
        console.warn('Google GIS prompt notice:', err.message)
      }
    }
    setProvider('google')
    setStep('GOOGLE_DIRECT')
        ...
        (response) => {
          if (response.authResponse?.accessToken) {
            ...
            fetch('/api/auth/facebook', {
              ...
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
              body: JSON.stringify({ accessToken: response.authResponse.accessToken, marketingOptIn }),
            })
              .then((r) => r.json())
              .then((data) => {
  ...
  // ── Xử lý form đăng nhập trực tiếp Google / Facebook ──────────
  const handleDirectOAuthSubmit = async (e, oauthProvider) => {
    e.preventDefault()
    setError(null)
    ...
    if (!trimmedName) {
      setError('Vui lòng nhập họ và tên của bạn để đặt tên tài khoản trên website.')
      return
    }
    ...
    setLoading(true)
    try {
      const endpoint = oauthProvider === 'facebook' ? '/api/auth/facebook' : '/api/auth/google'
  ...
  // ── Xử lý kiểm tra Mã xác minh 6 số (backward compatibility) ──
        ...
        <div className="bg-[#FAF8F5] text-[#1A1614] rounded-[4px] border border-[#D4AF37]/60 shadow-[0_12px_40px_rgba(99,21,33,0.18)] overflow-hidden font-sans relative">
          ...
          {step === null && (
            <div className="p-4 sm:p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 pr-4">
                  ...
                  <p className="text-xs text-[#4A3F38] mt-1 leading-relaxed">
                    Tạo tài khoản riêng trên website và nhận ngay ưu đãi <strong className="text-[#631521] font-semibold">giảm 10%</strong> + <strong className="text-[#631521] font-semibold">Miễn phí vận chuyển</strong> cho đơn đủ điều kiện đầu tiên.
                  </p>
                </div>
              </div>
              ...
              {/* Lựa chọn phương thức đăng nhập Google & Facebook */}
              <div className="mt-4 pt-3.5 border-t border-[#E8DFD5] space-y-2.5">
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 border border-[#D8D2C9] bg-white text-[#1A1614] hover:bg-[#FAF8F5] hover:border-[#631521] py-2.5 px-4 rounded-[2px] text-xs font-semibold tracking-wide shadow-xs cursor-pointer transition-all"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center gap-3 border border-[#1877F2] bg-[#1877F2] text-white hover:bg-[#166FE5] py-2.5 px-4 rounded-[2px] text-xs font-semibold tracking-wide shadow-xs cursor-pointer transition-all"
                >
                ...
                <p className="text-[11px] text-[#8C7E74] text-center pt-1 leading-snug">
                  Tên tài khoản trên website sẽ được tự động đồng bộ theo tên tài khoản Google hoặc Facebook của bạn.
                </p>
          ...
          {/* ── BƯỚC 2A: XÁC NHẬN TÀI KHOẢN GOOGLE ─────────────────── */}
          {step === 'GOOGLE_DIRECT' && (
            <div className="p-5">
              ...
              <p className="text-xs text-[#4A3F38] mb-3 leading-relaxed">
                Hệ thống sẽ tạo <strong>tài khoản riêng trên website</strong> cho bạn với tên tài khoản được lấy chuẩn xác theo tên Google:
              </p>

              <form onSubmit={(e) => handleDirectOAuthSubmit(e, 'google')} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#4A3F38] uppercase tracking-wider mb-1">
                    Tên tài khoản (Họ và tên Google) *
                  </label>
                  <div className="relative">
                    <User className="w-4 h-4 text-[#8C7E74] absolute left-3 top-3" />
                    <input
                      type="text"
                      value={fullName}
                      onChange={(e) => setFullName(e.target.value)}
                      placeholder="Ví dụ: Nguyễn Đức Quân"
                      required
                      className="w-full pl-9 pr-3 py-2 bg-white border border-[#E8DFD5] rounded-[2px] text-xs text-[#1A1614] focus:outline-none focus:border-[#631521]"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 py-2 bg-white border border-[#E8DFD5] rounded-[2px] text-xs text-[#1A1614] focus:outline-none focus:border-[#631521]"
                  />
                </div>

                <label className="flex items-start gap-2 pt-1 text-[11px] text-[#4A3F38] cursor-pointer select-none">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span>Tôi đồng ý nhận email thông báo ưu đãi độc quyền từ QuanNguyenS.</span>
                </label>
                ...
                >
                  {loading ? (
                    ...
                    <span>XÁC NHẬN TẠO TÀI KHOẢN & NHẬN VOUCHER</span>
                  )}
                </button>
              </form>
            </div>
          ...
          {/* ── BƯỚC 2B: XÁC NHẬN TÀI KHOẢN FACEBOOK ───────────────── */}
          {step === 'FACEBOOK_DIRECT' && (
            <div className="p-5">
              <div className="flex items-center justify-between pb-3 border-b border-[#E8DFD5] mb-3.5">
                <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-[#4A3F38] mb-3 leading-relaxed">
                Hệ thống sẽ tạo <strong>tài khoản riêng trên website</strong> cho bạn với tên tài khoản được lấy chuẩn xác theo tên Facebook:
              </p>

              <form onSubmit={(e) => handleDirectOAuthSubmit(e, 'facebook')} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold text-[#4A3F38] uppercase tracking-wider mb-1">
                    Tên tài khoản (Họ và tên Facebook) *
                  </label>
                  <div className="relative">
                    <User className="w-4 h-4 text-[#8C7E74] absolute left-3 top-3" />
                    <input
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-[#1877F2] hover:bg-[#166FE5] text-white text-xs font-bold uppercase tracking-wider py-2.5 px-4 rounded-[2px] transition-colors cursor-pointer flex items-center justify-center gap-1.5 disabled:opacity-60"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
          ...
          {/* ── BƯỚC 2C: NHẬP MÃ XÁC MINH 6 SỐ (NẾU ĐƯỢC YÊU CẦU) ───── */}
          {step === 'VERIFY_CODE' && (
            <div className="p-5">
              <div className="flex items-center justify-between pb-3 border-b border-[#E8DFD5] mb-3.5">
                <div className="flex items-center gap-2">
              ...
              <p className="text-xs text-[#4A3F38] mb-2 leading-relaxed">
                Mã xác minh bảo mật đã được gửi tới <strong>{targetDisplay}</strong>. Vui lòng kiểm tra hộp thư email và nhập mã để kích hoạt tài khoản:
              </p>
          ...
          {step === 'SUCCESS' && (
            <div className="p-5 text-center">
              ...
              <h4 className="font-serif text-lg font-bold text-[#1A1614]">
                Tài Khoản Đã Khởi Tạo Thành Công!
              </h4>
              <p className="text-xs text-[#4A3F38] mt-1 mb-3.5 leading-relaxed">
                Chào mừng <strong>{fullName || 'Quý khách'}</strong> đến với <strong>QuanNguyenS</strong>. Tài khoản riêng của bạn đã được kích hoạt. Dưới đây là mã ưu đãi độc quyền dành cho đơn đầu tiên của bạn:
              </p>
              ...
              <div className="space-y-2">
                ...
                <Link
                  to="/tai-khoan"
                  onClick={handleDismiss}
                  className="flex items-center justify-center gap-1.5 text-xs text-[#631521] hover:underline font-semibold pt-1"
                >
