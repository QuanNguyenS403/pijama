...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const resolvedParams = await params;
  const orderId = resolvedParams?.id || (params as any)?.id;
  ...
  try {
    ...
    switch (action) {
      case 'CONFIRM':
        updatedOrder = await OrderService.adminConfirmOrder(orderId, adminId, (rest as any).note);
        break;
      ...
      case 'CANCEL':
        updatedOrder = await OrderService.adminCancelOrder(orderId, adminId, (rest as any).reason);
        break;

      case 'NOTE':
        updatedOrder = await OrderService.updateAdminNote(orderId, (rest as any).note);
        break;
...
const ACTION_SUCCESS_MSG: Record<string, string> = {
