// lib/services/orderService.js — Business logic đơn hàng
...
export const OrderService = {
  ...
  async createOrder(input) {
    const { customer, shipping, items, paymentMethod,
            voucherCode, note, userId } = input
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      for (const item of items) {
        ...
        if (available < item.quantity) {
          throw new Error(
            `${variant.product.name} (${variant.colorLabel} - ${variant.size}) ` +
      ...
      const shippingFee   = subtotal >= parseInt(freeThreshold?.value || 500000) ? 0
                          : parseInt(defaultFee?.value || 30000)
  ...
  // Hủy đơn hàng
  async cancelOrder(orderId, reason, adminId) {
    return await prisma.$transaction(async (tx) => {
  ...
  // ════════════════════════════════════════════
  async getOrderDetail(orderIdOrNumber) {
  ...
  // ════════════════════════════════════════════
  async adminConfirmOrder(orderId, adminId, note = '') {
    const order = await prisma.order.findUnique({
      where:   { id: orderId },
      include: { items: { include: { product: true, variant: true } } },
    })
    ...
    NotificationService.updateSheetStatus(order.orderNumber, 'CONFIRMED', note).catch(console.error)
  ...
  async markAsProcessing(orderId, adminId) {
    ...
    NotificationService.updateSheetStatus(order.orderNumber, 'PROCESSING').catch(console.error)
  ...
  // ════════════════════════════════════════════
  async markAsShipped(orderId, adminId, trackingNumber, shippingNote = '') {
    const order = await prisma.order.findUnique({
      where:   { id: orderId },
      include: { items: { include: { product: true, variant: true } } },
    })
    ...
    // Email khách có tracking + cập nhật Sheet
    NotificationService.sendStatusUpdate(updated, 'SHIPPED', { trackingNumber }).catch(console.error)
    NotificationService.updateSheetStatus(order.orderNumber, 'SHIPPED', trackingNumber ? `Mã vận đơn: ${trackingNumber}` : '').catch(console.error)
  ...
  // ════════════════════════════════════════════
  async markAsDelivered(orderId, adminId) {
    const order = await prisma.order.findUnique({
      where:   { id: orderId },
      include: { items: true },
    })
  ...
  // ════════════════════════════════════════════
  async updateAdminNote(orderId, note) {
    return prisma.order.update({
      where: { id: orderId },
      data:  { adminNote: note },
    })
