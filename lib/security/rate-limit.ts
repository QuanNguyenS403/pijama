// lib/security/rate-limit.ts — Rate limiting dùng Redis counter
...
export interface RateLimitOptions {
  ...
  keyPrefix?: string;  // Tiền tố key Redis
}
...
export const rateLimit = ({
  ...
  return async (req: any): Promise<RateLimitResult> => {
    try {
      ...
      // Fallback cho phép request nếu Redis tạm thời mất kết nối (fail-open)
      console.warn(`[RateLimit Warning] Redis unavailable for key prefix ${keyPrefix}:`, error);
      return {
        isAllowed: true,
        remaining: max,
        reset: Date.now() + windowMs,
...
export const createRateLimitMiddleware = (limiter: ReturnType<typeof rateLimit>) => {
